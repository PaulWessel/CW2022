...
gmt begin
	...
	gmt select chain_ages/${chain1}_info.txt -i5,6,2 -Z0/6 | gmt convert -Td > rel_drift_rates/${chain1}.txt
	gmt math rel_drift_rates/${chain1}.txt -C0 DUP 0 LT 360 MUL ADD = rel_drift_rates/${chain1}.txt #no negative longitudes
	gmt select chain_ages/${chain2}_info.txt -i5,6,2 -Z0/6 | gmt convert -Td > rel_drift_rates/${chain2}.txt
	...
	# sample coeval chain 1 and chain 2 values and output lon, lat, age, sig width, sig along
	...
	gmt convert rel_drift_rates/${chain1}.txt tmp4.txt -A | gmt mapproject -i0,1,3,4 -fg -G+v+i+ud -o4 > tmp5.txt
	gmt convert rel_drift_rates/${chain2}.txt tmp3.txt -A | gmt mapproject -i0,1,3,4 -fg -G+v+i+ud -o4 > tmp6.txt
